...
const client = (program) => {
    ...
    if (typeof program.host === 'host') {
        client_host = program.host;
    }
    if (typeof process.env.HOST) {
        client_host = program.host;
    }
    ...
    console.log("Connect url: ", connect_url.href);
...
program
    ...
    .option('--family-name <familyName>', 'Family name, or last name, or the user')
...
program
    ...
    .option('--family-name <familyName>', 'New family name')
    .option('--given-name <givenName>', 'New given name')
    .option('--middle-name <middleName>', 'New middle name')
...
program
    ...
    .action((username, password, cmdObj) => {
        client(program).post('/password-check', { username, password }, (err, req, res, obj) => {
            if (err) console.error(err.stack);
            else console.log('Password check result= '+ util.inspect(obj));
        });
    });
